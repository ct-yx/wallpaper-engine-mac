...
> **项目状态：** 本项目不再继续开发。若你在寻找功能更完善的 macOS 实现，请前往 **[jipika/WaifuX](https://github.com/jipika/WaifuX)**。
...
### 场景文本、声音和更安全的 SteamCMD 导入

- **静态场景文本** — 现在可渲染场景 `text` 对象的内容、源字体（含包内字体）、字号、换行宽度、颜色、透明度、内边距、对齐、父子层级，以及初始 `{"value": ...}` 设置值。
- **场景声音对象** — `sound` 数组和单个声音路径都可从松散文件或 `.pkg` 资源播放；支持 `loop`、睡眠/唤醒行为以及共享的音量与播放速率控制。
- **完整缓存检查与回滚** — 只有 `project.json` 与可播放根资源均存在时才复用 SteamCMD 内容；导入会保留原本地副本，直到暂存替换成功。

## 0.9.9 新功能

### SteamCMD 会话流程

- **详情页不再探测会话** — 打开创意工坊条目或下载配置页时，不会为了验证已保存的登录状态而启动 SteamCMD。
- **记住会话后直接下载** — 先前成功登录过 Steam 后，详情页会立即提供“下载”操作；SteamCMD 只会在实际传输该条目时验证会话。
- **可恢复的过期会话** — 缓存会话过期时，条目会切换到需要配置下载的状态，不再反复重试 SteamCMD；下载输出的收集也已同步，能保留真实失败信息。
