...
### 場景文字、聲音與更安全的 SteamCMD 匯入

- **靜態場景文字** — 現在可繪製場景 `text` 物件的內容、來源字型（包含封包內字型）、字級、換行寬度、顏色、透明度、內距、對齊、父子階層，以及初始 `{"value": ...}` 設定值。
- **場景聲音物件** — `sound` 陣列和單一聲音路徑都可從鬆散檔案或 `.pkg` 資源播放；支援 `loop`、睡眠/喚醒行為以及共用的音量與播放速率控制。
- **完整快取檢查與回復** — 僅在 `project.json` 和可播放根資源皆存在時重用 SteamCMD 內容；匯入會保留既有本機副本，直到暫存替換成功。

## 0.9.9 新功能

### SteamCMD 工作階段流程

- **詳細頁面不再探測工作階段** — 開啟創意工坊項目或下載設定頁面時，不會只為確認已儲存的登入狀態而啟動 SteamCMD。
- **記住工作階段後直接下載** — 先前成功登入 Steam 後，詳細頁面會立即提供 **Download**；SteamCMD 只會在實際傳輸該項目時驗證工作階段。
- **可恢復的過期工作階段** — 快取工作階段過期時，項目會切換到需要設定下載的狀態，不再反覆重試 SteamCMD；下載輸出的收集也已同步，可保留實際失敗資訊。

## 0.9.8 新功能

### 場景變換與粒子生命週期一致性

- **粒子生命週期算子** — `sizechange`、`alphachange` 與 `colorchange` 現在會遵循作者設定的生命週期區間，在區間前後保持對應值，並如 Linux 參考渲染器般乘以初始尺寸、透明度或顏色。
- **巢狀場景變換** — 影像和粒子物件現在會保留父層群組、來源座標原點、縮放、角度、可見性與對齊錨點，不再把所有圖層扁平化到場景根節點。
- **彈性場景屬性** — 變換、顏色、尺寸、透明度及可見性皆支援 `{"value": ...}` 自訂包裝；材質中明確帶有圖片副檔名的紋理項目（如 `texture.png`）也能直接載入。

## 0.9.7 新功能

### 粒子動畫紋理

- **TEXS 粒子開始動畫** — 粒子材質現在會重用已解碼的 TEXS 多圖像／圖集影格和 `.tex-json` 精靈圖，而不再只顯示第一影格。
- **遵循來源動畫模式** — 迴圈、`once` 與 `randomframe` 粒子模式會依作者設定的影格時長與 `sequencemultiplier` 速度排程。
- **SpriteKit 相容回退** — SpriteKit 為每個發射器而不是每個存活粒子提供紋理時間軸，因此同一發射器會共用影格選擇；粒子移動、生命週期和既有發射器行為維持不變。

## 0.9.6 新功能

### SteamCMD 快取與遊戲庫可靠性

- **立即重用已下載內容** — 如果 SteamCMD 已下載某個創意工坊項目，詳細頁面會直接匯入快取檔案，不再啟動另一個 SteamCMD 行程，也不必先還原登入工作階段。
- **尋找全部已設定的 Steam 遊戲庫** — 除了標準 macOS、Homebrew 和獨立 SteamCMD 位置外，匯入器也會讀取 `libraryfolders.vdf`，因此能正確發現儲存在第二個或外接 Steam 遊戲庫中的內容。
- **持續使用原子匯入** — 快取內容和新下載內容都透過暫存目錄替換，避免半複製的桌布出現在本機遊戲庫中。

## 0.9.5 新功能

### TEXS 場景動畫紋理

- **支援多影格及圖集 TEXS** — TEX 解析器現在會走訪每個 TEXB 圖像，並讀取 TEXS0001/0002/0003 影格表，包含來源圖像索引、顯示時間和圖集裁切區域。
- **精靈圖中繼資料** — 靜態 `materials/<texture>.tex-json` 精靈圖序列也會在場景圖像圖層上播放；相容兩種常見的歷史附屬檔命名方式。
- **更多原生紋理格式** — 除了 DXT1/DXT3/DXT5 和內嵌 PNG/JPEG，現在可渲染原始 RGBA/RGB、RG88 和 R8 TEX mipmap。非視訊 TEXB0004 容器會使用與上游 v3 相容的 mipmap 配置。
- **已驗證的二進位解析** — 圖像、mipmap 和影格記錄均有邊界檢查，並已使用合成的多圖像 TEXS、圖集 TEXS、附屬中繼資料和原始 RGBA 夾具驗證。
...
- **粒子效果** — 已支援常見的 SpriteKit 相容發射器和共用 TEXS／精靈圖紋理動畫；進階粒子運算子、繩索／拖尾渲染器、控制點及依粒子選擇影格仍為近似實作或尚未支援。
...
- **動畫場景** — 已支援場景圖像和粒子圖層上的 TEXS 多圖像／圖集動畫與 `.tex-json` 精靈圖；依粒子選擇影格以及時間軸／腳本驅動的物件動畫尚未實作。
